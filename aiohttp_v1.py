...
async def parse_similar_videos(source_html) -> set:
...
async def fetch_many(concurrency: int, video_hashes: set, cnt: Counter=None) -> tuple:
    ...
    tasks = [asyncio.ensure_future(_task(hash, sem, cnt)) for hash in video_hashes]
    results = await asyncio.gather(*tasks)
    video_hashes = list(set(itertools.chain(*results)))
    return video_hashes, {}


async def _task(hash: str, sem: asyncio.Semaphore, cnt: Counter) -> set:
    out = set()
                    ...
                    async with session.get(url) as resp:
                        ...
                        if code == 200:
                            try:
                                ...
                                result = await parse_similar_videos(response_content)
                                if result:
